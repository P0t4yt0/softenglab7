...
class RegistrationFrame(ctk.CTkFrame):
    """
    Modernized registration form UI with validation, error messages, 
    and login redirection on success.
    """
    ...
    def submit_registration_placeholder(self):
        """Frontend validation + open login after success."""
        ...
        if not fname or not lname or not uname or not pwd or not cpwd or not email or not emp_id:
            ...
            return
        if len(uname) < 5:
            self.message_label.configure(text="⚠️ Username must be at least 5 characters.", text_color="red")
            return
        if pwd != cpwd:
            self.message_label.configure(text="⚠️ Passwords do not match.", text_color="red")
            return
        if not emp_id.isdigit() or len(emp_id) != 5:
            self.message_label.configure(text="⚠️ Employee ID must be 5 digits.", text_color="red")
            return
        if "@" not in email or "." not in email:
            self.message_label.configure(text="⚠️ Invalid email address.", text_color="red")
            return
        if terms != "on":
        ...
        # Close registration window and open login view
        self.master.destroy()  # close this window
        subprocess.Popen(["python", "login_view.py"])  # open login_view.py
